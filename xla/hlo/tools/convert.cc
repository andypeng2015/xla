...
absl::Status RealMain(absl::string_view input_file,
  ...
  TF_ASSIGN_OR_RETURN(
      const auto module_and_arguments,
      FunctionalHloRunner::LoadHloModuleAndArguments(input_file, input_format));
...
int main(int argc, char** argv) {
  ...
  absl::Status result =
      xla::tools::RealMain(argv[1], output_file, input_format, output_format);
  if (!result.ok()) {
